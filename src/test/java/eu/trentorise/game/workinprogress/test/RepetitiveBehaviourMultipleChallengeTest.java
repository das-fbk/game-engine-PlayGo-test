...
public class RepetitiveBehaviourMultipleChallengeTest extends GameTest{
	...
	public void initEnv() {
		...
		data.put("periodTarget", 1d); //min #weeks active to win the challenge
		...
		// assign another challenge to PLAYER
		data = new HashMap<String, Object>();
		data.put("target", 5d); //min #greanleaves to consider the day active
		data.put("counter", "green leaves");
		data.put("periodName", "daily");
		data.put("periodTarget", 2d); //min #weeks active to win the challenge
		data.put("bonusScore", 8d);
		data.put("bonusPointType", "green leaves");
		playerSrv.assignChallenge(GAME, PLAYER, "repetitiveBehaviour", "repetitiveBehaviourInstance2",
				data, dayBeforeYesterdayDate, tomorrowDate);
	...
	public void analyzeResult() {
		//PLAYER had 25 green leaves
		//She won just one challenges (100), so the final score should be 125
		assertionPoint(GAME, 125d, PLAYER, "green leaves");
	}
